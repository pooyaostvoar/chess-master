...
import type { BaseUser, CreatePeriodicBatchSlotsInput } from "@chess-master/schemas";
...
export function mapUserToSchemaBase(user: User): BaseUser {
  return {
    ...
    youtubeVideos: user.youtubeVideos ?? undefined,
...
export type CreatePeriodicBatchSlotsResult = {
  config: PeriodicSlotConfig;
  slots: ScheduleSlot[];
};

/**
 * Create a periodic slot config and insert chunked schedule slots for a master.
 */
export async function createPeriodicBatchSlots(
  masterId: number,
  input: CreatePeriodicBatchSlotsInput,
  manager: EntityManager = AppDataSource.manager
): Promise<CreatePeriodicBatchSlotsResult> {
  const {
    interval,
    chunkSizeMinutes,
    repeatCount,
  ...
  return manager.transaction(async (trx) => {
    ...
    return {
      config,
      slots: insertResult.raw as ScheduleSlot[],
    };
  });
}
