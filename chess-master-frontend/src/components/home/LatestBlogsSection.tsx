...
const PIECES = ["king", "queen", "knight", "pawn"] as const;

function getHeaderColor(id: number): string {
  return HEADER_COLORS[id % HEADER_COLORS.length];
}

function getPiece(id: number): (typeof PIECES)[number] {
  return PIECES[id % PIECES.length];
}
...
const PieceSvg: React.FC<{ piece: string }> = ({ piece }) => {
  ...
  return (
    <svg viewBox="0 0 45 45" width="70" height="70">
...
export const BlogCard: React.FC<BlogCardProps> = ({ post }) => {
  const headerBg = getHeaderColor(post.id);
  const colorIdx = post.id % 3;
  ...
  return (
    ...
    >
      ...
      <div className="px-3.5 py-3 pb-3.5">
        ...
        <span className="text-sm text-[#B8893D] font-medium">Read →</span>
      </div>
    </Link>
  );
};
...
export const LatestBlogsSection: React.FC<LatestBlogsSectionProps> = ({
  ...
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-3 gap-3 ${compact ? "" : ""}`}>
      {posts.map((post) => (
